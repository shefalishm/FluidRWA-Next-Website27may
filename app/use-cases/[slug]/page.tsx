...
export default async function UseCaseDetailPage({ params }: PageProps) {
  ...
  return <main className="uc-page uc-detail"><JsonLd data={jsonLd} />
    <section className="uc-detail-hero"><div className="uc-container"><a className="uc-back" href="/use-cases"><ArrowLeft size={15} /> Use cases</a><div className="uc-hero-grid"><div className="uc-hero-copy"><p className="uc-kicker">{item.industry}</p><h1>{guide.title}</h1><p className="uc-lead">{item.summary}</p><div className="uc-hero-meta"><span>For {guide.audience.toLowerCase()}</span><span>Reviewed by <a href="/about">FluidRWA Research Team</a></span></div><div className="uc-actions"><a className="uc-button" href="#workflow">Explore the workflow <ArrowRight size={18} /></a><a className="uc-text-link" href="/submit-requirement">Discuss your project <ArrowRight size={17} /></a></div></div><div className="uc-hero-image"><img src={item.image} alt={item.imageAlt} width={900} height={600} fetchPriority="high" /></div></div></div></section>
