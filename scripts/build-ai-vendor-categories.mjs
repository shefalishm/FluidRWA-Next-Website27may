...
const web3AiCategories = [
  ...
  },
  {
    ...
    vendors: [
      vendor("Revid.ai", "https://www.revid.ai/", "Social video creation", "Creates short-form videos from ideas, scripts, articles and assets, with social publishing workflows.", "Best for turning Web3 ideas into short videos quickly.", "Freemium / paid credits", ["TikTok", "Instagram", "YouTube Shorts"]),
...
function categoryImage(category) {
  const imageGroups = {
    "/assets/ai-vendor-video.webp": ["ai-video-generation-tools", "ai-voiceover-avatar-tools"],
    "/assets/ai-vendor-social.webp": ["ai-social-media-tools", "ai-marketing-content-tools", "ai-design-creative-tools"],
    "/assets/ai-vendor-workflow.webp": ["ai-sales-outreach-tools", "ai-crm-revops-tools", "ai-research-market-intelligence-tools", "ai-customer-support-tools", "ai-compliance-document-tools", "ai-document-intelligence-knowledge-retrieval"],
    "/assets/ai-vendor-infrastructure.webp": ["ai-automation-agent-tools", "ai-agents-autonomous-systems", "ai-risk-analytics-compliance-intelligence", "decentralized-ai-compute-gpu-infrastructure", "ai-data-model-marketplaces", "verifiable-ai-smart-contract-infrastructure"]
  };
  const match = Object.entries(imageGroups).find(([, slugs]) => slugs.includes(category.slug));
  return match ? match[0] : "/assets/ai-vendor-workflow.webp";
}
...
function heroVisual(categoryLabel = "AI vendor workflows", imageSrc = "/assets/ai-vendor-workflow.webp") {
...
function categoryPage(category) {
  ...
  const rows = category.vendors.map((v) => {
    ...
    const promoNote = v.affiliateStatus === "affiliate-active" && v.affiliateNote ? `<span class="ai-promo-note">${esc(v.affiliateNote)}</span>` : "";
