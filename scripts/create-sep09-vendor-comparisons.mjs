...
const guides = JSON.parse(fs.readFileSync(path.join(root, "scripts", "sep09-vendor-comparisons.json"), "utf8"));
...
${g.topicSections.map(([heading, body]) => `### ${heading}\n\n${body}`).join("\n\n")}
...
for (const g of guides) {
  const faqs = [
    ...
    ["How current is this comparison?", "It was reviewed on September 9, 2026 using publicly available primary vendor materials. Verify current availability directly before procurement."]
  ];
