...
const reports = [
  {
    title: "FluidRWA Digital Asset Infrastructure Research Brief",
    author: "FluidRWA Research Desk",
    description: "A shared research brief for teams exploring digital asset infrastructure, market development and the vendor ecosystem supporting tokenized finance.",
    href: "https://gemini.google.com/share/f617449055ca",
  ...
  },
  {
    ...
    description: "A market-structure report covering tokenization use cases, interoperability, regulation and the infrastructure required for the next generation of value exchange.",
  ...
  },
  {
    ...
    description: "A central-bank perspective on tokenisation, unified ledgers, settlement, programmable finance and the future architecture of money and financial markets.",
...
function updateSitemap(posts) {
  ...
  const entries = [`  <url><loc>${site}/blog.html</loc><lastmod>${reviewedDate}</lastmod><changefreq>weekly</changefreq><priority>0.85</priority></url>`, `  <url><loc>${site}/blog/tokenization/</loc><lastmod>${reviewedDate}</lastmod><changefreq>weekly</changefreq><priority>0.78</priority></url>`, ...posts.map((p) => `  <url><loc>${site}/blog/${p.slug}/</loc><lastmod>${reviewedDate}</lastmod><changefreq>monthly</changefreq><priority>0.72</priority></url>`)].join("\n");
